...
import imutils
import os
# sys.path.insert(0, '../object_detect/')
class stop_light_detector():
    def __init__(self):
        if(__debug__):
            print('stop_light_detector created')

        self.stop_light_cascade = cv2.CascadeClassifier('D:\\UCI\\Senior Design Project\\DSCubed\\object_detect\\TrafficLight_HAAR_16Stages.xml')
        self.stop_sign_cascade = cv2.CascadeClassifier('D:\\UCI\\Senior Design Project\\DSCubed\\object_detect\\Stopsign_HAAR_19Stages.xml')
    ...
    def detect_stop(self,image):
        try:
            ...
            w = 0
            h = 0
            stop_sign = self.stop_sign_cascade.detectMultiScale(image, 1.03, 5)
